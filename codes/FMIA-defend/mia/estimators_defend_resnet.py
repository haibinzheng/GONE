...
class ShadowModelBundle(sklearn.base.BaseEstimator):
    ...
    def fit_transform(self, X, y, verbose=False, fit_kwargs=None):
        ...
        """
        self._fit(X, y, verbose=verbose, fit_kwargs=fit_kwargs)#Train the shadow models.
        return self._transform(verbose=verbose)#Produce in/out data for training the attack model.
    ...
    def _fit(self, X, y, verbose=False, pseudo=False, fit_kwargs=None):
        ...
        indices = np.arange(X.shape[0])#0-8999
        ...
        for i in self._get_model_iterator(verbose=verbose):
            ...
            )#0-8999,8000个，采样中无重复值
            train_indices = shadow_indices[: self.shadow_dataset_size]#0-8999,4000
            test_indices = shadow_indices[self.shadow_dataset_size :]#1-8997,4000
            X_train, y_train = X[train_indices], y[train_indices]#4000个
