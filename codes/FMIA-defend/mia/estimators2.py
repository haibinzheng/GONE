...
def softmax_defend(x,epi):
    return np.exp(epi*x/2) / np.sum(np.exp(epi*x/2))
...
def prepare_attack_data2(model, data_in, data_out):
    """
    Prepare the data in the attack model format.
    ...
    y_hat_in = model.predict_proba(X_in)
    y_hat_out = model.predict_proba(X_out)
    label_in = np.argmax(y_hat_in, axis=1)

    h, w = y_hat_in.shape
    # 标准正态分布*noise_sigma
    noise = np.random.randn(h, w) * noise_sigma
    y_hat_in = y_hat_in + noise
    y_hat_out = y_hat_out + noise
    p1 = np.argmax(y_hat_in, axis=1)
...
def prepare_attack_data_target(model, data_in, data_out):
    """
    Prepare the data in the attack model format.
    ...
    logit_matrix_in = []
    logit_matrix_out = []
    confidence_matrixs_in = []
    confidence_matrixs_out = []
    x =[]
    with foolbox.models.KerasModel(model, bounds=(0, 1), channel_axis=1) as model1:
        for i, image in enumerate(X_in):
            print("epi:%d", epi)
            logit_matrix_in = model1.predictions(image)

            confidence_matrix_in = softmax_defend(logit_matrix_in, epi)
            # np.save("/home/NewDisk/sgwc/attack/mia-master/examples/logit.npy", logit_matrix_in)
            # np.save("/home/NewDisk/sgwc/attack/mia-master/examples/confidence.npy", confidence_matrix_in)
            confidence_matrixs_in.append(confidence_matrix_in)
        for i, image in enumerate(X_out):
            logit_matrix_out = model1.predictions(image)
            confidence_image = softmax_defend(logit_matrix_out, epi)
            a = 2
            confidence_matrix_in = softmax_defend(confidence_matrix_in, a)
            confidence_matrixs_out.append(confidence_image)
    # back_func = KTF.function(inputs=[model.layers[0].input], outputs=[model.layers[5].output])
    # intermediate_layer_model = Model(inputs=model.input, outputs=model.get_layer(index=-1).output)  # 你创建新的模型
    # # 这个新的模型截取了原始模型中从输入到某个层（你想要得到结果的那一层）的一部分，
    # intermediate_output = intermediate_layer_model.predict(X_in)  # 这个数据就是原始模型的输入数据，

    y_hat_in = np.array(confidence_matrixs_in)
    # y_hat_in = model.predict_proba(X_in)
    y_hat_out = np.array(confidence_matrixs_out)
